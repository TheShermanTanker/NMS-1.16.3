...
/*     */         } else {
/* 221 */           throw new YAMLException("Keys must be scalars but found: " + tuple
/* 222 */               .getKeyNode());
/*     */         } 
...
/* 234 */             throw new YAMLException("No writable property '" + key + "' on class: " + beanType
/* 235 */                 .getName());
/*     */           }
/*     */           
...
/*     */ 
/*     */           
/* 269 */           Object value = (memberDescription != null) ? newInstance(memberDescription, key, valueNode) : Constructor.this.constructObject(valueNode);
/*     */ 
/*     */           
...
/*     */           }
/*     */           
/* 283 */           if (memberDescription == null || 
/* 284 */             !memberDescription.setProperty(object, key, value)) {
/* 285 */             property.set(object, value);
/*     */           }
/* 287 */         } catch (DuplicateKeyException e) {
/* 288 */           throw e;
/* 289 */         } catch (Exception e) {
/* 290 */           throw new ConstructorException("Cannot create property=" + key + " for JavaBean=" + object, node
/*     */               
/* 292 */               .getStartMark(), e.getMessage(), valueNode.getStartMark(), e);
/*     */         } 
/*     */       } 
/* 295 */       return object;
/*     */     }
...
/* 335 */         throw new ConstructorException(null, null, "Can't construct a java object for " + node
/* 336 */             .getTag() + "; exception=" + e.getMessage(), node.getStartMark(), e);
/*     */       } 
/*     */     }
/*     */     
/*     */     public void construct2ndStep(Node node, Object object) {
...
/* 344 */         throw new ConstructorException(null, null, "Can't construct a second step for a java object for " + node
/*     */             
/* 346 */             .getTag() + "; exception=" + e.getMessage(), node
/* 347 */             .getStartMark(), e);
/*     */       } 
/*     */     }
/*     */   }
...
/* 363 */       } catch (InstantiationException null) {
/*     */         Object result;
/*     */ 
/*     */         
/* 367 */         if (type.isPrimitive() || type == String.class || Number.class.isAssignableFrom(type) || type == Boolean.class || Date.class
/* 368 */           .isAssignableFrom(type) || type == Character.class || type == BigInteger.class || type == BigDecimal.class || Enum.class
/*     */           
/* 370 */           .isAssignableFrom(type) || Tag.BINARY
/* 371 */           .equals(node.getTag()) || Calendar.class.isAssignableFrom(type) || type == UUID.class) {
/*     */ 
/*     */           
/* 374 */           Object object = constructStandardJavaInstance(type, node);
/*     */         } else {
/*     */           Object argument;
/*     */           
...
/* 392 */               throw new YAMLException("No single argument constructor found for " + type + " : " + ie
/* 393 */                   .getMessage());
/*     */             }  
...
/* 409 */               throw new YAMLException("Can't construct a java object for scalar " + node
/* 410 */                   .getTag() + "; No String constructor found. Exception=" + e
/* 411 */                   .getMessage(), e);
/*     */             } 
/*     */           } 
/*     */           try {
...
/* 418 */             throw new ConstructorException(null, null, "Can't construct a java object for scalar " + node
/* 419 */                 .getTag() + "; exception=" + e
/* 420 */                 .getMessage(), node
/* 421 */                 .getStartMark(), e);
/*     */           } 
/*     */         } 
...
/* 443 */             throw new YAMLException("Invalid node Character: '" + ch + "'; length: " + ch
/* 444 */                 .length());
/*     */           }
...
/* 480 */           result = Byte.valueOf(result.toString());
/* 481 */         } else if (type == Short.class || type == short.class) {
/* 482 */           result = Short.valueOf(result.toString());
...
/* 496 */           throw new YAMLException("Unable to find enum value '" + enumValueName + "' for enum class: " + type
/* 497 */               .getName());
/*     */         } 
...
/*     */ 
/*     */       
/* 549 */       List<Constructor<?>> possibleConstructors = new ArrayList<>(snode.getValue().size());
/* 550 */       for (Constructor<?> constructor : node.getType()
/* 551 */         .getDeclaredConstructors()) {
...
/*     */         } 
/*     */       } 
/* 604 */       throw new YAMLException("No suitable constructor with " + 
/* 605 */           String.valueOf(snode.getValue().size()) + " arguments found for " + node
/* 606 */           .getType());
/*     */     }
/*     */ 
/*     */ 
...
/* Location:              C:\Users\Josep\Downloads\Decompile Minecraft\tuinity-1.16.3.jar!\org\yaml\snakeyaml\constructor\Constructor.class
