...
/*  96 */       for (Iterator<Map.Entry<String, Entries>> i = this.resolveCache.entrySet().iterator(); i.hasNext(); ) {
...
/* 109 */     Entries entries = this.resolveCache.remove(hostname);
...
/* 117 */     Entries entries = this.resolveCache.get(hostname);
...
/* 125 */     Entries entries = this.resolveCache.get(hostname);
/* 126 */     if (entries == null) {
/* 127 */       entries = new Entries(hostname);
/* 128 */       Entries oldEntries = this.resolveCache.putIfAbsent(hostname, entries);
...
/* 169 */       super(Collections.emptyList());
...
/* 219 */               scheduleCacheExpirationIfNeeded(ttl, loop); return;
/*     */             }  continue;
/*     */           } 
/* 222 */           if (compareAndSet(entries, Collections.singletonList(e))) {
...
/*     */       while (true) {
/* 238 */         ScheduledFuture<?> oldFuture = Cache.FUTURE_UPDATER.get(this);
/* 239 */         if (oldFuture == null || oldFuture.getDelay(TimeUnit.SECONDS) > ttl) {
/* 240 */           ScheduledFuture scheduledFuture = loop.schedule(this, ttl, TimeUnit.SECONDS);
/*     */ 
/*     */ 
/*     */ 
/*     */ 
/*     */ 
/*     */           
/* 247 */           if (Cache.FUTURE_UPDATER.compareAndSet(this, oldFuture, scheduledFuture)) {
...
/*     */             break;
/*     */           } 
/* 254 */           scheduledFuture.cancel(true);
/*     */           continue;
/*     */         } 
/*     */         break;
/*     */       } 
...
/* 263 */       List<E> entries = getAndSet(Collections.emptyList());
/* 264 */       if (entries.isEmpty()) {
/* 265 */         return false;
/*     */       }
/*     */       
/* 268 */       ScheduledFuture<?> expirationFuture = Cache.FUTURE_UPDATER.getAndSet(this, Cache.CANCELLED);
...
/*     */     
/*     */     public void run() {
/* 287 */       Cache.access$200(Cache.this).remove(this.hostname, this);
/*     */       
/* 289 */       clearAndCancel();
/*     */     }
/*     */   }
